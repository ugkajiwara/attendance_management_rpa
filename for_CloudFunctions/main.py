...
def sendMail(noticesForMail,date):    # メール送信処理
    # 送信元情報
    SENDER_EMAIL = '##送信元メアド##'
    ...
    server.sendmail(SENDER_EMAIL, toList, message.as_string())
    server.close()


def errMail(date):    # エラーメール送信処理
    # 送信元情報
    SENDER_EMAIL = '##送信元メアド##'
    SENDER_PASSWORD = '##アプリパスワード##'
    SENDER_DOMAIN = 'smtp.gmail.com'
    ...
    # 送信するメールの内容
    to = '##送信先メアド１## ,##送信先メアド２##'
    sub = '!!エラー!!【〇〇店】' + date + 'のシフト記録お知らせ'
    body = '処理中にエラーが発生したため、' + date + 'のシフト記録は手打ちしてください。'
    ...
    try:
        ...
        for name in names: #namesを回す処理
            ...
            if help_store != '': #ヘルプ店舗の場合スキップ
                ...
                notices.append('別店舗ヘルプのため' + name + 'の入力をスキップします')
                continue
        ...
        if notices != []:
            for notice in notices:
                noticesForMail = noticesForMail + notice + '\r\n'

        # ブラウザを終了する
        driver.close()
        driver.quit()

        sendMail(noticesForMail,date)
        print('メールを送信しました')
